...
class MonitorINotify(Monitor):
    def __init__(self, args:ArgumentParser, queue:queue.Queue, config:Config) -> None:
        Monitor.__init__(self, "Mon", args, config)
        self.__qIn = queue
...
class MonitorPolling(Monitor):
    def __init__(self, args:ArgumentParser, dirname:str, config:Config) -> None:
        Monitor.__init__(self, "Poll", args, config)
