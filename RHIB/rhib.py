...
class RHIB_Parser:
    ...
    def __navinfo(self, box:int, t:datetime.datetime, tail:bytes, line:bytes) -> int:
        ...
        lat = float(info[1])
        lon = float(info[2])
        sql = "INSERT INTO rhibNAV VALUES(%s,%s,%s,%s)"
        sql+= " ON CONFLICT DO NOTHING"
        sql+= ";"
        self.__cursor.execute(sql, (self.__ship, t, lat, lon));
        return 1
    ...
    def __keelctd(self, box:int, t:datetime.datetime, tail:bytes, line:bytes) -> int:
        ...
        temp = float(info[8])
        SP = float(info[9])
        sql = "INSERT INTO rhibCTD VALUES(%s,%s,%s,%s)"
        sql+= " ON CONFLICT DO NOTHING"
        sql+= ";"
        self.__cursor.execute(sql, (self.__ship, t, temp, SP));
        return 1
    ...
    def __adcp(self, box:int, t:datetime.datetime, tail:bytes, line:bytes) -> int:
        ...
        if u.size != v.size or u.size != w.size:
            return 0
        sql = "INSERT INTO rhibADCP VALUES(%s,%s,%s,%s,%s)"
        sql+= " ON CONFLICT DO NOTHING"
        sql+= ";"
        self.__cursor.execute(sql, (self.__ship, t, u.tolist(), v.tolist(), w.tolist()));
        return 1
